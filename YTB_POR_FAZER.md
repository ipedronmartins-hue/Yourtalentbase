...
- **6 políticas RLS com `auth.uid()` não envolvido em `SELECT`.** Em Postgres/Supabase, `USING (auth.uid() = coluna)` reavalia a função **uma vez por linha**; `USING ((SELECT auth.uid()) = coluna)` avalia uma só vez por query. Verificado em produção: 6 das 71 políticas do schema `public` têm o padrão não otimizado. **Impacto hoje: nenhum** — com uma dezena de atletas a diferença não é mensurável. Passa a contar quando alguma tabela chegar à ordem dos milhares de linhas (provavelmente `atleta_eventos` primeiro, que é a que mais cresce). Correção é mecânica e sem risco de comportamento: só muda o plano de execução, não o resultado. *Origem: única coisa aproveitável de uma revisão do repositório open-source ECC (`affaan-m/ECC`, 281 skills) — o resto não se aplicava ao stack nem cobria o domínio (nada sobre RGPD, menores ou consentimento).*
- **Escala de tipografia por tokenizar no resto do site.** O fundador reparou que a plataforma "parece desorganizada" comparada com o Windows Phone (poucos tamanhos de letra, título sempre garrafal). Medido: **~800 `font-size:Npx` soltos em 29 ficheiros, mais de 40 valores distintos**, na maioria deriva de meio-pixel sem desenho nenhum por trás. Criados 6 tokens em `ytb-tokens.css` (`--fs-hero/titulo/sub/corpo/legenda/micro`) e aplicados a `ytb-pro-treinador.html`, `admin360.html` e `passaporte.html` (as três páginas do piloto) — zero `font-size:Npx` a mais nestes três ficheiros, exceto uma exceção deliberada (o temporizador do cronómetro em `passaporte.html`, 84px, precisa de ser legível ao longe durante o exercício). **As restantes ~26 páginas continuam com a escala antiga**: `index.html`, `como-funciona.html`, `sobre.html`, `coachbase.html`, `scouts.html`, `clube.html`, `acompanhamento.html`, `elite-coach.html`, `montra.html`, `registar.html`, `auth.html`, `acesso.html`, `inscrever.html`, `relatorio.html`, `scouting360.html`, `scouting360-dashboard.html`, `ocultar.html`, `privacidade.html`, `termos.html`, e as páginas já identificadas como ilha morta (`treinador.html`, `pais.html`, `plataforma.html` — ver §3.3). Aplicar aos tokens é mecânico (mesmo processo, script de mapeamento por valor+contexto, já usado 3 vezes) mas mexe em CSS visível em toda a página — fazer com o mesmo cuidado, uma página de cada vez, fora da pressão do piloto.
