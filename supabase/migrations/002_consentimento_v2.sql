-- ============================================================================
-- MIGRAÇÃO 001 · CONSENTIMENTO V2
-- ----------------------------------------------------------------------------
-- Âmbitos separados, versionados, controlados PELA FAMÍLIA, com revogação real.
-- Sincroniza os flags operacionais existentes (visivel_b2b, visivel_publico,
-- foto_consentida, video_consentido) para não partir nenhuma query atual.
-- Cada mudança emite evento no livro-razão (histórico completo).
-- Constituição: princípio 6 · nunca-fazemos 1 e 3.
-- ============================================================================

-- ── PRÉ-REQUISITO: constraints reais de produção precisam de 2 valores novos ──
-- estado: +'revogado' (revogação total da família); fonte: +'auto' (inscrição livre,
-- bug latente herdado: o CHECK antigo rejeitava o que a RPC deployada escrevia).
...
-- ── helper interno: é o encarregado deste atleta? ───────────────────────────
...
-- ── estado dos consentimentos (vista da família) ────────────────────────────
...
begin
  ...
  -- sincronizar os flags operacionais (nenhuma query existente parte)
  if p_ambito = 'visibilidade_b2b' then
    ...
    -- REVOGAÇÃO TOTAL: sai de tudo, imediatamente
  ...
  -- evento no livro (mudanças de consentimento são UPDATE-heavy → emissão aqui)
...
-- ── revogação total (atalho explícito, prometido no ecrã) ───────────────────
...
-- ── consentimento de primeiro acesso (v2): registo + b2b opcional ───────────
...
begin
  ...
  if not v_ja then
    ...
    -- montra_publica fica FALSE por omissão: é opt-in explícito da família, no passaporte
  end if;

  return jsonb_build_object('ok',true,'id',v_id,'nome',v_nome,'ja_consentido',v_ja);
end $$;

-- ── montra pública v2: SEM rating (Constituição · nunca 1) + guardas ─────────
