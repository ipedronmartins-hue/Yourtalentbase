...
-- Quando o plano prescrito acumula 12+ execuções da família (um ciclo de
-- 4 semanas × 3 sessões), ytb_plano_proximo gera a continuação: mesmo plano,
-- fonte='auto', herdando o treinador original (proveniência preservada).
-- Idempotente por construção: a nova prescrição zera a contagem. O evento
-- treino_prescrito é emitido pelo trigger existente; ytb_o_que_mudou passa
-- a distinguir "plano do treinador" de "plano de continuação". O digest do
-- treinador mostra o plano auto e o treinador pode substituí-lo a qualquer
-- momento — o miúdo nunca fica sem caminho seguinte, o treinador mantém a
-- autoridade.
-- Validada em dry-run transacional (2026-07-04): não gera sem 12 execuções ✓,
-- gera com 12 ✓, proveniência herdada ✓, evento ✓, idempotente ✓, mensagem
-- de continuação ✓, intruso bloqueado ✓. (O dry-run apanhou o NOT NULL real
-- de treinador_id em produção — resolvido com a herança de proveniência.)
-- ============================================================================
...
begin
  ...
  -- a continuação herda o treinador do plano original (proveniência);
  -- fonte='auto' marca o mecanismo de geração
